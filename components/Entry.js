...
const Entry = ({entry}) => {
    const {title,synopsis,image,published_at,id} = entry
  return (
    <article>
        {/* priority was added because there was a warning about loading time for images and recommended using priority */}
        <Image src={image.url} priority='true' layout='responsive' width={800} height={570} alt={`Image for blog entry: ${title}`}/>
